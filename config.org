  ...
  (if (eq system-type 'windows-nt)
      (progn (setq ccls-executable "ccls.exe")
              (setq ccls-initialization-options
                    `(:cache (:directory "./_build/ccls-cache")
...
(use-package! sbt-mode
  ...
  (advice-add 'sbt-command :after #'(lambda (x) (other-window -1))))
  (advice-remove 'sbt-command #'(lambda () (other-window -1)))
#+end_src
...
(defun ento/open-terminal-in-working-dir ()
    ...
    (interactive)
    (call-process-shell-command "wt powershell.exe" nil 0))
...
(defun ento/run-skipperlite-in-working-dir ()
    "Open terminal in current working directory"
    (interactive)
    (call-process-shell-command "wt skipperlite-async.exe" nil 0))
...
#+begin_src emacs-lisp :tangle ~/.doom.d/config.el :filename config.el
(defun ento/download-firmwares (device)
    "Download firmwares to defualt directory for DEVICE"
    (interactive "sDevice firmware to downlaods: ")
    (message device)
    (let ((concat_dev (prin1-to-string device)))
                     (call-process-shell-command (concat "wt python.exe C:\\Git\\collection-of-scripts\\download_fw_from_EC.py --extract_path C:\\Firmwares\\ --log_file C:\\Git\\dci\\ --device " concat_dev) nil 0)))
...
(defun ento/run-rico ()
    ...
    ()
    (let ((concat_dev (prin1-to-string device)))
                     (call-process-shell-command (concat "wt python.exe C:\\Git\\collection-of-scripts\\download_fw_from_EC.py --extract_path C:\\Firmwares\\ --device " concat_dev) nil 0)))
